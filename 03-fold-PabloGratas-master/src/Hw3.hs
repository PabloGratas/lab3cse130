...
sqSum xs = foldLeft f base xs
  where
   f a x = a + x*x
   base  = 0 -- This is the base case for our sum, should be obvious we start summing at 0. If the list has no elements or has all zero elements, we want to return zero.
